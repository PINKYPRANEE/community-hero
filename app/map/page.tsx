...
export default function MapPage() {
  ...
  const getPosition = (issue: any, index: number) => {
            ...
            <div className="rounded-2xl overflow-hidden shadow">
              <LoadScript googleMapsApiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY || ""}>
                <GoogleMap mapContainerStyle={mapContainerStyle} center={center} zoom={13}>
                  {issues.map((issue, index) => (
                    <Marker key={issue.id} position={getPosition(issue, index)} onClick={() => setSelected(issue)} label={getIcon(issue.category)} />
                  ))}
                  {selected && (
                    <InfoWindow position={getPosition(selected, issues.indexOf(selected))} onCloseClick={() => setSelected(null)}>
                      <div className="p-2">
                        ...
                        <span className={`mt-2 inline-block px-2 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          selected.status === 'In Progress' ? 'bg-yellow-100 text-yellow-600' : 'bg-red-100 text-red-600'
